...
import { DragItem, Point, DropResult } from './mercury-types'
...
export interface UseMercuryDragResult {
  ...
  dragPreview: (node: HTMLElement | null) => void
}
    ...
    drop: (item: DragItem, monitor) => {
      ...
      if (clientOffset) {
        ...
        if (dropElement) {
          ...
          console.log('🎯 CARD WILL BE CENTERED ON:', `x: ${relativePosition.x}px, y: ${relativePosition.y}px from viewport top-left`)
          
          onDrop(item, relativePosition)
        } else {
          console.warn('Drop target not found, using client offset as fallback')
 ...
 */
export function createDragItemFromListing(listing: any, sourceContext: string = 'housing-module'): DragItem {
  return {
    id: listing.id,
 ...
 */
export function createDragItemFromAction(action: any, sourceContext: string = 'action-popup'): DragItem {
  return {
 ...
 */
export const getMercuryDropZoneStyles = (isOver: boolean, canDrop: boolean) => ({
  backgroundColor: isOver && canDrop ? 'rgba(59, 130, 246, 0.08)' : 'transparent',
