...
export const MERCURY_FOCUS_LEVELS = {
  focused: {
    ...
    animation: 'transition-all duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]',
...
export const MERCURY_COMPONENT_PADDING = {
...
export const MERCURY_ANIMATIONS = {
  ...
  focus: 'focus-visible:ring-2 focus-visible:ring-slate-300/40 focus-visible:ring-offset-2'
} as const
 ...
 */
export function getMercuryFocusClasses(focusLevel: 'focused' | 'ambient' | 'fog') {
 ...
 */
export function getMercuryStatusColors(status: 'healthy' | 'warning' | 'critical' | 'neutral') {
 ...
 */
export function getMercuryTypography(
  focusLevel: 'focused' | 'ambient' | 'fog',
  size: 'compact' | 'standard' | 'expanded'
) {
 ...
 */
export function getMercuryAccentBar(
  focusLevel: 'focused' | 'ambient' | 'fog',
  status: 'healthy' | 'warning' | 'critical' | 'neutral'
) {
  if (focusLevel === 'focused') {
    return MERCURY_ACCENT_BARS.focused[status as keyof typeof MERCURY_ACCENT_BARS.focused] || 
...
export function validateMercuryProps(props: MercuryComponentProps): string[] {
  ...
  if (props.focusLevel && !['focused', 'ambient', 'fog'].includes(props.focusLevel)) {
