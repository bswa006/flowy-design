...
const getEnterpriseStatusColors = (status: EnterpriseMetric['status']) => {
  return getMercuryStatusColors(status || 'neutral')
}
...
export function EnterpriseDashboardCard({
  ...
  const handleAction = useCallback((actionIntent: string) => {
  ...
  const sizeClasses = {
    compact: MERCURY_COMPONENT_PADDING.compact,
    standard: MERCURY_COMPONENT_PADDING.standard,
    expanded: MERCURY_COMPONENT_PADDING.expanded
  }
  
  const StatusIcon = data.status === 'healthy' ? CheckCircle2 :
                   data.status === 'warning' ? AlertCircle :
                   data.status === 'critical' ? AlertCircle : Activity
  ...
  return (
    <div
      ...
      className={cn(
        ...
        data.status === 'warning' && focusLevel === 'focused' && 'animate-pulse-gentle',
        data.status === 'critical' && focusLevel === 'focused' && 'animate-pulse-soft',
    ...
    >
        ...
        <div className="mb-6">
          <div className="flex items-baseline space-x-4 mb-3">
            <span className={cn(
              // Mercury typography from tokens
              getMercuryTypography(focusLevel, size),
              "tabular-nums tracking-tight leading-none",
              statusColors.text
            )}>
              {typeof data.value === 'number' ? animatedValue.toLocaleString() : data.value}
            </span>
         ...
         {data.previousValue && typeof data.value === 'number' && (
           <p className="text-sm text-slate-500">
             Previous: {typeof data.previousValue === 'number' ? data.previousValue.toLocaleString() : data.previousValue}
           </p>
         )}
       </div>
          ...
          {data.actions.map((action, index) => (
            <button
              ...
              className={cn(
                ...
                action.priority === 'primary' && 'bg-blue-600 text-white hover:bg-blue-700',
                action.priority === 'secondary' && 'border border-slate-200 hover:border-slate-300',
