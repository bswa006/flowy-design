...
const analyzeSelectedText = (text: string, context: string) => {
  const _trimmedText = text.trim().toLowerCase();
  const actions: Array<{
    id: string;
    title: string;
...
const SAMPLE_MESSAGES: ChatMessage[] = [
  ...
  },
  {
    ...
    author: "Devin",
    content:
      "The second one is literally called 777 Expense Way I am SCREMING sdjkafffjdjfdjdj",
    avatar: "https://i.pravatar.cc/40?u=devin",
  },
  {
    id: "3",
    author: "Victoria",
    content:
      "Jason did you rig the results or something there's no way everything is this expensive –",
  ...
  },
  {
    ...
    author: "Marisa",
    content:
      "Look at the search query it's literally \"Find 4 bedroom apartments in San Francisco\" I'm really not trying to be slick y'all",
    avatar: "https://i.pravatar.cc/40?u=marisa",
  },
  {
    id: "6",
    author: "Marisa",
    content:
      "brb changing the playlist because we can't keep listening to Blackpink",
    avatar: "https://i.pravatar.cc/40?u=marisa",
  },
  {
    id: "7",
    author: "Victoria",
    content:
      "tbh we should just look in Mountain View instead because SF is just SF",
    avatar: "https://i.pravatar.cc/40?u=victoria",
  },
];
...
export function MercuryChatModule({
  ...
  const [modalAbsolutePosition, setModalAbsolutePosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  const handleTextSelection = useCallback(
    (event: React.MouseEvent, messageId: string) => {
      ...
      const actions = analyzeSelectedText(
        selectedTextContent,
        `message-${messageId}`
      );
      ...
      let relativeX = chatRect
        ? rect.left + rect.width / 2 - chatRect.left
      ...
      if (chatRect) {
        ...
        relativeX = Math.max(
          padding + popupWidth / 2,
          Math.min(chatRect.width - padding - popupWidth / 2, relativeX)
        );
      ...
      setAvailableActions(
        actions.filter((action) => !usedActions.has(action.id))
      );
  ...
  const markActionAsUsed = useCallback(
    (actionId: string) => {
                          ...
                          >
                            <div
                              className={`w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 ${
                                action.type === "housing-search"
                                  ? "bg-red-100 group-hover:bg-red-200/80"
                            ...
                            >
                              <div
                                className={`rounded-full transition-all duration-200 ${
                                  action.type === "housing-search"
                                    ? "w-3 h-3 bg-red-500 group-hover:bg-red-600"
                                    : action.type === "location"
                                      ? "w-3 h-3 bg-blue-500 group-hover:bg-blue-600"
                                      : action.type === "person"
                                        ? "w-3 h-3 bg-green-500 group-hover:bg-green-600"
                    ...
                    {availableActions.length === 0 && (
                      ...
                      >
                        <p className="text-sm text-slate-600">
                          No actions available for this text
                        </p>
